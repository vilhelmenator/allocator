...
int main(int argc, const char * argv[]) {
    ...
    t2 = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::milliseconds>( t2 - t1 ).count();
    std::cout << "Time spent in malloc: " << duration << " milliseconds" << std::endl << std::endl;
    
    
    int sizes[] = {2, 200, 8, 311, 16, 612, 32, 48};
    std::cout << "Starting to test the custom new and delete calls..." << std::endl;
    t1 = std::chrono::high_resolution_clock::now();
    int k = 0;
    for(int j=0;j<NUMBER_OF_ITERATIONS; j++)
    {
    ...
    t2 = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::milliseconds>( t2 - t1 ).count();
    std::cout << "Time spent using normal new/delete calls: " << duration << " milliseconds" << std::endl << std::endl;
     ...
     */
    /*
    std::cout << "Starting to test the mimalloc new and delete calls..." << std::endl;
    k = 0;
    t1 = std::chrono::high_resolution_clock::now();
    for(int j=0;j<NUMBER_OF_ITERATIONS; j++)
    {
